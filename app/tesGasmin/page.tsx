...
interface GeofenceArea {
  ...
  longitude: number;
  radius: number;
  media: string; // Mengganti nama properti videoId menjadi media dan tipenya string
}
...
const Page = () => {
  ...
  const [currentMedia, setCurrentMedia] = useState<string | null>(null); // Mengganti currentVideoId menjadi currentMedia
  const [isPlaying, setIsPlaying] = useState(false);
  const [player, setPlayer] = useState<any>(null);
  const [currentAreaIndex, setCurrentAreaIndex] = useState<number | null>(null);

  const geofenceAreas: GeofenceArea[] = [
    { latitude: -6.9167322, longitude: 107.6613635, radius: 10, media: "/music/habibi.mp3" }, // Mengganti videoId menjadi media dan memberikan alamat file MP3
    { latitude: -6.9166387, longitude: 107.6615271, radius: 10, media: "36YnV9STBqc" },
    { latitude: -6.9167608, longitude: 107.6616099, radius: 10, media: "DOOrIxw5xOw" },
    { latitude: -6.9168766, longitude: 107.6614897, radius: 10, media: "XnUNOaxw6bs" },
  ];
        ...
        (position) => {
          ...
          geofenceAreas.forEach((area, index) => {
            ...
            if (distance <= area.radius) {
              ...
              areaIndex = index;
              if (currentMedia !== area.media) { // Mengganti currentVideoId menjadi currentMedia
                setCurrentMedia(area.media);
              }
            }
          });
          ...
          if (isInsideAnyGeofence) {
            ...
            setCurrentMedia("/music/habibi.mp3"); // Mengatur file MP3 yang akan diputar saat di luar area
          }
        },
        (error) => {
  ...
  useEffect(() => {
    if (player && currentMedia) { // Mengganti currentVideoId menjadi currentMedia
      if (currentMedia.endsWith(".mp3")) { // Memeriksa apakah media berupa file MP3
        player.loadMedia({
          contentId: currentMedia,
          contentType: 'audio/mp3',
        });
      } else {
        player.loadVideoById(currentMedia);
      }
      if (isPlaying) {
        player.playMedia(); // Mengganti playVideo menjadi playMedia
      } else {
        player.pauseMedia(); // Mengganti pauseVideo menjadi pauseMedia
      }
    }
  }, [player, currentMedia, isPlaying]);
          ...
          <div className="flex flex-col gap-4 items-center  pt-[16svh]">
            ...
            <div className="flex">
              {geofenceAreas.map((area) => (
                <React.Fragment key={area.media}>
                  {area.media.endsWith(".mp3") ? ( // Memeriksa apakah media berupa file MP3
                    <audio src={area.media} controls={false} autoPlay={false} />
                  ) : (
                    <YouTube
                      videoId={area.media}
                      onReady={onReady}
